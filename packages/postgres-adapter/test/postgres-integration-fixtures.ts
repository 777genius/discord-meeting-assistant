...
export function usePostgresIntegrationDatabase(): void {
  ...
  beforeEach(async () => {
    if (pool !== undefined) {
      await pool.query(
        "TRUNCATE TABLE meeting_core.legacy_historical_admissions, meeting_routing.source_configurations, meeting_core.recording_publication_reconciliations, meeting_core.derived_greeting_obligations, meeting_core.conversation_greeting_capacity_admissions, meeting_core.conversation_one_shot_receipts, meeting_core.answer_effects, meeting_knowledge.current_question_policy, meeting_knowledge.question_message_tombstones, meeting_knowledge.question_rate_reservations, meeting_knowledge.question_jobs, meeting_knowledge.unavailable_final_projections, meeting_knowledge.withdrawn_meeting_sources, meeting_core.historical_coverage_checkpoints, meeting_core.historical_memory_sync, meeting_knowledge.live_memory_hot_tail, meeting_knowledge.live_memory_outbox, meeting_knowledge.live_memory_meetings, meeting_core.summary_publication_effects, meeting_core.post_call_dead_letters, meeting_core.live_meeting_summary_coverage, meeting_core.live_meeting_turns, meeting_core.live_meeting_generation_usage, meeting_core.live_meeting_generation_telemetry, meeting_core.live_meetings, meeting_core.post_call_outbox, meeting_core.meetings",
      );
    }
  });
