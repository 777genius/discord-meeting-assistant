...
const commonKnowledgeAnswerSystemPrompt = [
  ...
  "Before emitting the first JSON key, decide privately whether the evidence answers the question.",
  "The schema emits claims before status: when the evidence answers the question, claims MUST already contain 1..12 cited items; never emit claims=[] and then status=answered.",
