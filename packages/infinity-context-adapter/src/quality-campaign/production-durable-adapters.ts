...
import { verifyExternalSignedValue, type ArtifactReceipt, type AttemptIdentity,
...
export class DurableAttemptJournal implements AttemptJournalPort {
  ...
  Promise<JournalRecovery> {
    ...
    if (terminal !== null) {
      ...
      const signed = verifyExternalSignedValue<ProviderTerminalPayload>(record.signedResult,
