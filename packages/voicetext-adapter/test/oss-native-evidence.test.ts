...
function journal(maximumBytes?: number) {
  ...
  return { directory, sink: new OssNativeEvidenceJournal({ directory,
...
describe("native OSS session journal", () => {
  it("retains a failed actual connector attempt without retaining its secrets", async () => {
    ...
    await expect(adapter.openSession({ meetingId: "meeting-1", speakerId: "speaker-1",
      idempotencyKey: "session-1", onTranscript: () => {} })).rejects.toThrow();
    sink.seal();
  ...
  it("never seals a truncated capture and never overwrites the journal", () => {
    ...
    expect(() => new OssNativeEvidenceJournal({ directory, project: "vtoss-test-oss-8f49a06-r1",
      revision: "a".repeat(40), testOnly: true })).toThrow();
  });
  it("does not seal an open session", () => {
    const { directory, sink } = journal();
    sink.open().record({ type: "opening", meetingId: "meeting", speakerId: "speaker",
  ...
  it("denies production admission", () => {
    expect(() => new OssNativeEvidenceJournal({ directory: "/tmp", project: "production",
      testOnly: true, revision: "a".repeat(40) })).toThrow("TEST admission");
  });
});
