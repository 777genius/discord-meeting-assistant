...
type TranscriptionStep = Error | StagePortResult<GeneratedTranscript>;
...
class SequenceTranscriber implements FinalTranscriptionPort {
  ...
  ): Promise<StagePortResult<GeneratedTranscript>> {
...
type SummaryStep = Error | StagePortResult<GeneratedSummary>;
...
class SequenceSummarizer implements SummaryGenerationPort {
  ...
  ): Promise<StagePortResult<GeneratedSummary>> {
    this.requests.push(structuredClone(request));
    const step = this.steps.shift();
    if (step === undefined) {
...
type PublicationStep = Error | StagePortResult<PublicationValue>;
...
class SequencePublisher implements SummaryPublicationPort {
  ...
  ): Promise<StagePortResult<PublicationValue>> {
    this.requests.push(structuredClone(request));
    const step = this.steps.shift();
    if (step === undefined) {
...
function success<Value>(value: Value): StagePortResult<Value> {
  return { ok: true, value };
}

function retryableFailure(code: string): StagePortResult<never> {
  return {
