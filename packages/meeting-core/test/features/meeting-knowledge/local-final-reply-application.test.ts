...
const authority: CurrentFinalReplyBinding = {
...
const selectedTurns: readonly RehydratedEvidenceTurn[] = Object.freeze([
  Object.freeze({
    endMs: 2_000,
...
const references: readonly FocusedMemoryReference[] = Object.freeze(
...
class AuthorizationFake implements QuestionAuthorizationPort {
  readonly checkpoints: QuestionAuthorizationCheckpoint[] = [];
  denyAt?: QuestionAuthorizationCheckpoint;
...
class EvidenceFake implements FinalReplyEvidencePort {
  current: CurrentFinalReplyBinding | null = authority;
  currentResult: CurrentFinalReplyBindingResult = {
    binding: authority,
    status: "current",
  };
  hydrated: CanonicalFinalReplyEvidenceResult = {
    binding: authority,
    status: "current",
    turns: selectedTurns,
  };
  ...
  findCurrentBinding(): Promise<CurrentFinalReplyBinding | null> {
    return Promise.resolve(this.current);
  }

  recheckCurrentBinding(): Promise<CurrentFinalReplyBindingResult> {
    this.rechecks += 1;
...
class MemoryFake implements FocusedMemoryRetrievalPort {
  calls: Parameters<FocusedMemoryRetrievalPort["retrieve"]>[0][] = [];
  historicalAuthorizationCalls = 0;
  historicalAuthorizationResults: boolean[] = [];
  result: FocusedMemoryRetrievalResult = {
  ...
  retrieve(input: Parameters<FocusedMemoryRetrievalPort["retrieve"]>[0]) {
    this.calls.push(input);
    return Promise.resolve(this.result);
  }

  reauthorizeHistoricalEvidence(): Promise<boolean> {
...
function binding(): QuestionBindingSnapshot {
...
describe("ProcessFinalReplyJob", () => {
  ...
  it("returns a fixed unavailable response for malformed provider citations", async () => {
    ...
    await expect(processor.executeOnce()).resolves.toMatchObject({ outcome: "unavailable" });
    expect(publication.reservations[0]?.content).toContain("currently unavailable");
