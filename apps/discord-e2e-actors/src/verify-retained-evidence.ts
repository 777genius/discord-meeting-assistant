...
async function main(): Promise<void> {
  const [manifestPath, evidencePath, thresholdsPath, ...extraPaths] = process.argv.slice(2);
  if (manifestPath === undefined || evidencePath === undefined) {
    throw new Error("Usage: verify-retained-evidence <manifest.json> <retained-evidence.json> [service-level-thresholds.json]");
  }
  if (extraPaths.length > 0) {
    throw new Error("Usage: verify-retained-evidence <manifest.json> <retained-evidence.json> [service-level-thresholds.json]");
  }

  const [manifestJson, evidenceJson, thresholdsJson] = await Promise.all([
    readFile(manifestPath, "utf8"),
    readFile(evidencePath, "utf8"),
    thresholdsPath === undefined ? undefined : readFile(thresholdsPath, "utf8"),
  ]);
  ...
  const serviceLevelThresholds = thresholdsJson === undefined
    ? undefined
    : serviceLevelThresholdsSchema.parse(JSON.parse(thresholdsJson));
