...
const startSchema = z.object({
  ...base, type: z.literal("capture_start"),
  kind: z.literal("oss-native-post-call-v1"), revision
}).strict();
const sealSchema = z.object({ ...base, type: z.literal("capture_seal"), priorSha256: digest }).strict();
const eventSchema = z.object({
  ...base, type: z.enum(["started", "succeeded", "failed", "threw"]),
  stage: z.enum(["transcription", "summary", "publication"]), meetingId: id, attempt: time.positive()
}).strict();
