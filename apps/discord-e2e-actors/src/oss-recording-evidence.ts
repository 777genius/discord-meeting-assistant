...
const manifestSchema = z.object({
  ...
  tracks: z.array(speakerAudio.omit({ audioLocator: true }).extend({
    locator: id,
...
const completionSchema = z.object({
  ...
  recording: z.object({
    authoritativeDurationMs: time, manifestChecksumSha256: digest,
    manifestLocator: id, manifestRevision: id, manifestSizeBytes: time, recordingId: id,
    speakerAudio: z.array(speakerAudio).length(2)
  }).strict(),
  authoritativeTracks: z.array(z.object({
    artifactVersionId: id, audioLocator: id,
    checksumSha256: digest, sizeBytes: time, speakerId: id, timelineOffsetMs: time,
    trackNumber: z.number().int().positive(), uploadId: id
  }).strict()).length(2),
}).strict();
...
export function verifyOssRecording(archive: Archive, run: OssRun): void {
  ...
  check(inventory.craigRevision === archive.plan.target.craigRevision &&
    ...
    same(inventory.files.map(({ path }) => path).toSorted(), [...run.originals].toSorted()),
  ...
  function verifyRecordingIdentity(): void {
  ...
  function verifyCompletion(): void {
    ...
    for (const lifecycle of run.lifecycle) {
      ...
      if (lifecycle.type === "recording.authoritative_ready") {
check(events[0]!.eventId === completion.finalEventId &&
