...
export const voicetextSemanticCanaryReceiptV1Schema = z.object({
  ...
  quality: z.object({
    characterErrorRate: z.number().finite().min(0).max(1),
    maximumCharacterErrorRate: z.number().finite().min(0).max(1),
    ...
    wordErrorRate: z.number().finite().min(0).max(1),
    maximumWordErrorRate: z.number().finite().min(0).max(1),
  }).strict(),
