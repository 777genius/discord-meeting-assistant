...
const composeCoordinate = z.string().regex(/^[a-z0-9][a-z0-9_-]{0,62}$/u);
const sha256 = z.string().regex(/^[a-f\d]{64}$/u);
const containerId = sha256;
...
const inputSchema = z.object({
  ...
  composeCanonical: z.string().min(1).max(1024 * 1024),
  composeCanonicalSha256: sha256,
  composeFile: absolutePath,
  credentialFile: absolutePath,
  database: z.object({
    imageIdentity: z.object({ imageId, repositoryDigest }).strict(),
    migrations: z.array(z.object({ checksum: sha256, version: identifier }).strict()).min(1).max(512),
    migrationTable: composeCoordinate,
    name: composeCoordinate,
    password: z.string().regex(/^[A-Za-z0-9_-]{32,128}$/u),
    schema: composeCoordinate,
    service: composeCoordinate,
    user: composeCoordinate,
    volume: composeCoordinate,
  }).strict(),
  migrationService: composeCoordinate,
  migrationImageIdentity: z.object({ imageId, repositoryDigest }).strict(),
  ...
  serviceIdentity: z.object({
    ...
    protocol: z.discriminatedUnion("kind", [
      z.object({ command: z.tuple([z.literal("/app/bin/craig-control"), z.literal("readiness"),
        z.literal("--format=json")]), expectedResponseSha256: sha256,
      ...
      z.object({ command: z.array(z.string().min(1).max(512)).min(1).max(16),
        expectedResponseSha256: sha256, kind: z.literal("test-port-substitute"),
        name: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u), version: identifier }).strict(),
    ]),
  }).strict(),
}).strict();

export type CraigCampaignStackInput = z.input<typeof inputSchema>;
...
export async function provisionCraigDisposableCampaignStack(
  ...
  await ports.mutationJournal.markStarted(Object.freeze({ campaignId: input.campaignId,
    ...
    projectName: plan.projectName, release: input.release, schemaVersion: 1 }));
...
export function craigProjectName(campaignId: string, release: HostedCampaignReleaseReferenceV1): string {
  ...
  const suffix = digestCanonical({ campaignId, release: parsedRelease }).slice(0, 20);
  return `craig-e2e-${suffix}`;
}

function credentialContents(input: z.infer<typeof inputSchema>, projectName: string): string {
  const environment = credentialEnvironment(input, projectName);
  return [...Object.entries(environment).map(([name, value]) => `${name}=${value}`), ""].join("\n");
}

function credentialEnvironment(input: z.infer<typeof inputSchema>, projectName: string): Record<string, string> {
...
function migrationRunArguments(input: z.infer<typeof inputSchema>): readonly string[] {
