...
export async function runOssCampaignCommand(args: readonly string[]) {
  ...
  requireEvidence((mode === "check" || mode === "qualify" || mode === "verify") && nonempty(planPath) && nonempty(root) && nonempty(manifestPath) &&
    nonempty(receiptPath) && extra.length === 0,
  ...
  return {
    kind: "oss-discord-stt-verification", status: receipt.status,
