...
export async function campaignFixture(root: string) {
  ...
  const runs = plan.runs.map((planned, i) => {
    ...
    const turns = manifest.fixtures.map((fixture, j) => ({ turnId: `turn-${i}-${j}`,
    ...
    const actor = { schemaVersion: 1, fixtureSetId: manifest.fixtureSetId, runId: planned.runId,
    ...
    const summary = { summaryId: `outline-${sha256(`meeting-${i}\0${summaryKey}\0${transcript.transcriptId}`).slice(0, 32)}`, transcriptId: transcript.transcriptId,
    ...
    const run = runSchema.parse({ kind: "oss-discord-stt-run-v1", ...planned, campaignId: plan.campaignId,
      ...
      originals: [original], tracks: turns.map((turn, j) => ({ speakerId: turn.speakerId,
        path: put(`recording-${i}.track-${j}.ogg`, "object-storage", fixtureAudio[j]),
        originalPaths: [original], timelineOffsetMs: offsets[j] })), transcript, liveTurns: turns,
      sessions: turns.map((turn, j) => {
        const sessionId = `session-${i}-${j}`;
        const wire = wireSchema.parse({ sessionId, recordingId: `recording-${i}`, meetingId: `meeting-${i}`,
        ...
        wires.push(wire);
        return { sessionId, speakerId: turn.speakerId, sourceRevision: plan.target.gatewayRevision,
          wirePath: put(`${sessionId}.json`, "gateway", wire) };
      }), summary,
      stages: ["transcription", "summary", "publication"].map((stage, j) => ({ stage, status: "succeeded",
        startedAtMs: ended + 10 + j * 10, completedAtMs: ended + 20 + j * 10 })),
      publication: { messageId: `message-${i}`, channelId: plan.target.resultsChannelId,
      ...
      settled: [200, 300].map((delay) => ({ observedAtMs: ended + delay, terminalAtMs: ended + 100,
    ...
    const manifestValue = { schemaVersion: 1, recordingId: run.recordingId, guildId: plan.target.guildId,
      ...
      tracks: run.tracks.map((track, j) => ({ speakerId: track.speakerId, locator: track.path,
    ...
    put(run.completionPath, "postgres", { schemaVersion: 6, lifecycleSchemaVersion: 1,
      ...
      events: run.lifecycle.map((event, j) => ({ type: event.type, occurredAt: new Date(event.atMs).toISOString(),
        eventId: `event-${j}`, digest: "f".repeat(64) })),
      recording: { recordingId: run.recordingId, authoritativeDurationMs: ended - started,
        manifestChecksumSha256: sha256(manifestBytes), manifestLocator: run.manifestPath,
        manifestRevision: "offline-version-1", manifestSizeBytes: manifestBytes.length, speakerAudio: acceptedTracks },
    ...
    put(run.originalInventoryPath, "craig", { recordingId: run.recordingId, craigRevision: plan.target.craigRevision,
    ...
    put(run.databasePath, "postgres", { matchingMeetingCount: 1, matchingRecordingCount: 1,
      matchingSummaryCount: 1, matchingTranscriptCount: 1, snapshot: {
        ...
        recording: { recordingId: run.recordingId, manifestLocator: run.manifestPath,
          manifestRevision: "offline-version-1", manifestSizeBytes: manifestBytes.length,
          manifestChecksumSha256: sha256(manifestBytes),
          speakerAudio: run.tracks.map((track, j) => ({ speakerId: track.speakerId, audioLocator: track.path,
  ...
  const deployment = { target: plan.target, targetAfter: structuredClone(plan.target), beforeMs: 0,
  ...
  const save = async () => {
    ...
    for (const [path, file] of files) {
      ...
      artifacts.push({ path, sha256: sha256(bytes), size: bytes.length,
        source: { system: file.system, locator: path, version: "offline-version-1" } });
    }
    const index = { kind: "oss-discord-stt-collection-v1", planSha256: sha256(planBytes),
