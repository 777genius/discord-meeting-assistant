...
function firewallRules(extraForward = ""): string {
  return `# Generated by iptables-save v1.8\n*filter
:FORWARD DROP [4:400]
:CRAIG_E2E - [0:0]
${extraForward}-A FORWARD -i br-craige2e -s 172.28.0.2/32 -j CRAIG_E2E
[9:900] -A FORWARD -o br-craige2e -d 172.28.0.2/32 -j CRAIG_E2E
-A CRAIG_E2E -s 172.28.0.2/32 -i br-craige2e -p tcp -m conntrack --ctstate NEW,ESTABLISHED --dport 443 -j ACCEPT
...
describe("concrete SSH deployment safety runner", () => {
  ...
  it("rejects wrong source, earlier shadowing, broad bypass, and a rule swap", () => {
    ...
    expect(() => proveCraigFirewallPolicy({ ...base, firewall: firewallRules().replace(
      "-A CRAIG_E2E -j RETURN", "-A CRAIG_E2E -j ACCEPT\n-A CRAIG_E2E -j RETURN",
    ) })).toThrow("exact TCP 443");
    expect(() => proveCraigFirewallPolicy({ ...base, firewall: firewallRules().replace(
      "-o br-craige2e -d 172.28.0.2/32", "-o br-craige2e -d 172.28.0.3/32",
    ) })).toThrow();
    expect(() => proveCraigFirewallPolicy({ ...base, firewall: firewallRules().replace(
      "-A CRAIG_E2E -s", "-A CRAIG_E2E -j RETURN\n-A CRAIG_E2E -s",
    ).replace("\n-A CRAIG_E2E -j RETURN\nCOMMIT", "\nCOMMIT") })).toThrow("terminal unconditional return");
  });
