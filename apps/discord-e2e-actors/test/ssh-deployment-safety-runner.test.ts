...
:CRAIG_E2E - [0:0]
:CRAIG_E2E_INPUT - [0:0]
-A INPUT -i br-craige2e -s 172.28.0.2/32 -j CRAIG_E2E_INPUT
...
describe("concrete SSH deployment safety runner", () => {
  ...
  it("rejects wrong source, earlier shadowing, broad bypass, and a rule swap", () => {
    ...
    expect(() => proveCraigFirewallPolicy({ ...base, firewall: firewallRules().replace(
      "-A CRAIG_E2E_INPUT -j DROP", "-A CRAIG_E2E_INPUT -j ACCEPT\n-A CRAIG_E2E_INPUT -j DROP",
    ) })).toThrow("host-escape chain");
    expect(() => proveCraigFirewallPolicy({ ...base, firewall: firewallRules().replace(
