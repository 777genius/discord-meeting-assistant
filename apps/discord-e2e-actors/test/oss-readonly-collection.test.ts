...
async function setup() {
  ...
  const command: OssReadCommand = async (args) => {
    ...
    if (args[0] === "inspect") {
      ...
      return JSON.stringify({ composeProject: fixture.plan.target.project,
    ...
    if (args.includes("-e") && args.some((arg) => arg.includes("GetObjectCommand"))) {
      ...
      return JSON.stringify(paths.map((path) => {
        ...
        return { locator: path, revision: "offline-version-1", sizeBytes: bytes.length,
...
describe("OSS read-only native source collection", () => {
  it("executes only admitted bounded reads and retains parsed native bytes", async () => {
    ...
    try {
      const result = await collectOssReadonlySnapshot({ plan: test.fixture.plan,
  ...
  it("denies collection before any database call for a non-TEST source", async () => {
    ...
    try {
      test.deny();
      await expect(collectOssReadonlySnapshot({ plan: test.fixture.plan,
        recordingId: test.run.recordingId, command: test.command })).rejects.toThrow("TEST admission");
