...
const mocks = vi.hoisted(() => ({ deployment: vi.fn(), snapshot: vi.fn(), docker: vi.fn(), publication: vi.fn(),
...
async function setup() {
  ...
  mocks.docker.mockImplementation(async (args: string[]) => {
    if (args[0] === "inspect") return JSON.stringify(args.at(-1) === services[0]!.containerId
  ...
  mocks.deployment.mockImplementation(async ({ phase }: { phase: string }) => {
    if (phase === "after") for (const name of ["live-native.jsonl", "post-call-native.jsonl"])
      await writeFile(join(journals, name), Buffer.concat([header, Buffer.from('{"type":"capture_seal"}\n')]));
  ...
  for (const run of f.runs) {
    ...
    for (const kind of ["data", "header1", "header2", "users", "info", "log"])
      await writeFile(join(craig, `${run.recordingId}.ogg.${kind}`), Buffer.from(kind));
    control.push(JSON.stringify({ runId: run.runId, recordingId: run.recordingId,
  ...
  mocks.snapshot.mockImplementation(async ({ recordingId }: { recordingId: string }) => {
    ...
    return { database: db, completion: { events: [{ type: "meeting.started", occurredAt: new Date(run.startedAtMs).toISOString() }] },
  ...
  mocks.originals.mockImplementation(async ({ jobBytes }: { jobBytes: Buffer }) => {
    ...
    return { files: ["data", "header1", "header2", "users", "info", "log"].map((kind) => ({
...
it("independently collects all runtime sources before binding retained bytes and creating a distinct PASS", async () => {
  ...
  const assembly = mocks.assemble.mock.calls[0]![0];
