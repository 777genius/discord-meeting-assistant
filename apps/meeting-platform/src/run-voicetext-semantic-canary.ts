...
function requiredBatchProfile(value: string): VoicetextBatchProfile {
  if (!isVoicetextBatchProfile(value)) {throw new Error("Voicetext semantic canary batch profile is invalid");}
  return value;
}
function requiredLiveProfile(value: string): VoicetextLiveProfile {
  if (!isVoicetextLiveProfile(value)) {throw new Error("Voicetext semantic canary live profile is invalid");}
  return value;
}
...
function exactDeadlineMs(value: string): number {
  ...
  if (!Number.isSafeInteger(deadlineMs) || deadlineMs > 300_000) {throw new Error("Voicetext semantic canary deadline is invalid");}
  return deadlineMs;
}
