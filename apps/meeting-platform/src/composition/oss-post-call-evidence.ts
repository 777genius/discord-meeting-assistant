...
export class OssPostCallEvidence {
  ...
  public wrap(input: {
    ...
    return {
      transcriber: { transcribe: (request) => this.measure("transcription", request.meetingId,
        () => input.transcriber.transcribe(request)) } satisfies FinalTranscriptionPort,
      summarizer: { generate: (request) => this.measure("summary", request.meetingId,
        () => input.summarizer.generate(request)) } satisfies SummaryGenerationPort,
      publisher: { publish: (request) => this.measure("publication", request.meetingId,
        () => input.publisher.publish(request)) } satisfies SummaryPublicationPort,
    };
  }
  public seal(): void {
