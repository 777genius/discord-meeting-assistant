...
| `KEEP_IN_DISCORD_PROFILE_CUSTODY` | `apps/meeting-platform/src/config.ts`: `environmentSchema`, `participantProfilesHaveNoConsumer`; `apps/meeting-platform/src/config/participant-greeting-profiles.ts`: `participantSpeakerAliases` | Discord profile custody retains configured names and maps profiles to canonical opaque actor keys. Meeting Knowledge, not Infinity, resolves aliases in question text. | `config.test.ts` profile sharing and no-consumer rejection; local alias privacy tests. | Keep. Never send names, Discord IDs, or the alias map upstream. |
