...
# ADR-0041: Fence binding recovery from legacy workers

## Status

Accepted on 2026-08-18. This decision extends ADR-0040 with a database-level
recovery fence and an explicit rollback boundary.

## Context

ADR-0040 separates new work onto the V2 queue, but a pre-binding worker can
...
After migration 0027, reverting application code without reverting durable
...
- A code revert to a pre-migration binary is unsupported after migration 0027.
