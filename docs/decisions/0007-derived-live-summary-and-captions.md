...
- Publish one Discord thread/message after five minutes, then edit it at most
  once per five-second refresh. The message contains a preliminary summary and
  a separate bounded `Сейчас говорят` embed with speaker mentions and timings.
...
- Admit exactly `gpt-5.6-luna` with low reasoning for the incremental purpose.
...
- Use one canonical Discord projection identity based only on meeting and target
  channel. Operation idempotency remains separate. Live finalization is fenced
  before post-call dispatch so the authoritative final projection always wins.
